...
class FetchPrototypeCommand extends Command
{
    ...
    public const ARTIFACT_NAME_REGEX = '/^keeperfx\-([\d\_\-]+?)[\-\_]Prototype\_([a-f\d]+?)\-patch$/';
    ...
    protected function execute(Input $input, Output $output): int
    {
        ...
        $workflow_id = (int) ($_ENV['APP_PROTOTYPE_GITHUB_WORKFLOW_ID'] ?? 0);
        ...
        foreach ($runs as $run) {
            ...
            $filetype     = \end($exp);
            $new_filename = $artifact->name . '-' . \substr(\sha1(\time() . $artifact->name), 0, 8) . '.7z';
